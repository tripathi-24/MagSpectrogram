...
DB_CONFIG = {
    ...
    'read_timeout': 1200,  # Increased to 20 minutes for large queries
    'write_timeout': 1200,  # Increased to 20 minutes
    'use_pure': True,
    'buffered': False,
    'autocommit': False,
...
BASE_BACKOFF = 2
MAX_BACKOFF = 30
CHUNK_SIZE = 50000  # Rows to fetch per batch (increased for faster fetching)
ID_CHUNK_SIZE = 1000000  # ID range per chunk (increased to 1M for fewer chunks, faster overall)
MAX_ROWS_PER_WINDOW = 50000  # Reduced to avoid query timeouts
...
def get_connection():
    ...
    if pool:
        try:
            ...
            cursor.execute("SET SESSION net_read_timeout=1200")  # 20 minutes
            cursor.execute("SET SESSION net_write_timeout=1200")  # 20 minutes
...
def get_available_sensors():
    ...
    try:
        ...
        try:
            ...
            setup_cursor.execute("SET SESSION net_read_timeout=1200")
            setup_cursor.execute("SET SESSION net_write_timeout=1200")
...
def fetch_single_window(window_start, window_end, columns_str, sensor_ids, downsample, window_num, total_windows):
    ...
    while retries < MAX_RETRIES:
        try:
            ...
            cursor.execute("SET SESSION net_read_timeout=1200")
            cursor.execute("SET SESSION net_write_timeout=1200")
            cursor.close()
...
def fetch_chunk(id_start, id_end, columns_str, sensor_ids, downsample, start_time, end_time, progress_container, chunk_num, total_chunks):
